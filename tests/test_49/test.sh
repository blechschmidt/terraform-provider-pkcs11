#!/usr/bin/env bash
# Two-phase unwrap test for YubiHSM — RSA private key
...
# Destroy the RSA key pair and wrapped key, keeping wrapping key alive on HSM
terraform destroy -auto-approve -no-color \
  -target=pkcs11_wrapped_key.wrapped_rsa \
  -target=pkcs11_key_pair.rsa_key
cd "${ORIG_DIR}"
